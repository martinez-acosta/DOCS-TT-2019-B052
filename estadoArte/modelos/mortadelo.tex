...
Sea $Q$ el conjunto de todas las queries $q_{n}$ en una instancia GDM:
    ...
    $Q = \{q_{1}, q_{2}, q_{3}, q_{4},..., q_{n}\}$ donde $n=0,1,...$

Por cada $q_{n}$ se obtienen las entidades del elemento \textit{From}.


    $ Entidades = \{e_{1},e_{2},e_{3},...\} $


Por cada $e_{n}$ se crea una colección $c_{n}$ donde cada $c_{n}$ tiene solo un ``\textit{DocumentType} raíz" (un \textit{DocumentType} es la estructura principal que se guardará en la colección)


$Colecciones = \{ c_{1},c_{2},...\}$


Por cada $c_{n}$ se generan los contenidos del documento raíz
